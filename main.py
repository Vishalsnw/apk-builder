...
from kivy.app import App from kivy.uix.boxlayout import BoxLayout from kivy.uix.button import Button from kivy.uix.label import Label from kivy.uix.textinput import TextInput from kivy.uix.popup import Popup

Constants for file paths

FOLDER_PATH = "/storage/emulated/0/KalyanApp" CSV_PATH = os.path.join(FOLDER_PATH, "data.csv") SCALER_PATH = os.path.join(FOLDER_PATH, "scaler.pkl") MODEL_PATH = os.path.join(FOLDER_PATH, "model.h5") ENCODER_PATH = os.path.join(FOLDER_PATH, "label_encoder.pkl")

class KalyanApp(BoxLayout): def init(self, **kwargs): super().init(orientation='vertical', **kwargs)

self.date_input = TextInput(hint_text='Enter date (dd/mm/yyyy)')
    self.market_input = TextInput(hint_text='Enter market (e.g., Kalyan)')
    self.open_input = TextInput(hint_text='Enter Open value')
    self.jodi_input = TextInput(hint_text='Enter Jodi value')
    self.close_input = TextInput(hint_text='Enter Close value')
...
class KalyanMLApp(App): def build(self): return KalyanApp()

if name == 'main': KalyanMLApp().run()
